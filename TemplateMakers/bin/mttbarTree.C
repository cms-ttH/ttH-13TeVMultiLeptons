...
//       else if( sample==2605 ) wgt = 7.87 * intLumi * 1./809984; //NNLO
	...
	sample = 2500;
      ...
      TLorentzVector topLV, topBarLV, topTotalLV;
      TLorentzVector muonLV;
      TLorentzVector neutrinoLV;
      TLorentzVector WLV;
      TLorentzVector WtransLV;
      
      unsigned numTops = 0, numMuons =0, numNeutrinos =0, numW =0;
      for( unsigned i=0; i< mcparticles.size(); i++ ){
        ...
        if ( id == 6 ){
          topLV.SetPxPyPzE(mcparticles.at(i).px,
                           mcparticles.at(i).py,
                           mcparticles.at(i).pz,
                           mcparticles.at(i).energy);
          numTops++;
                           
        } else if (id == -6) {
          topBarLV.SetPxPyPzE(mcparticles.at(i).px,
                              mcparticles.at(i).py,
                              mcparticles.at(i).pz,
                              mcparticles.at(i).energy);
          numTops++;
          ...
          // check that this is from a W not a b
          if (abs(motherId) != 24 ) continue;
          muonLV.SetPxPyPzE(mcparticles.at(i).px,
                            mcparticles.at(i).py,
                            mcparticles.at(i).pz,
                            mcparticles.at(i).energy);
          ...
          // is this a W from top?
          if (abs(motherId) != 6) continue;

          // how did it decay? leptonically?
          int daughter0Id = mcparticles.at(i).daughter0Id;
          int daughter1Id = mcparticles.at(i).daughter1Id;

          // if none of the daughter are W's, then skip this 
          if (abs(daughter0Id) != 13 && abs(daughter1Id) !=13) continue;
          
          WLV.SetPxPyPzE(mcparticles.at(i).px,
                            mcparticles.at(i).py,
                            mcparticles.at(i).pz,
                            mcparticles.at(i).energy);
          numW++;
        } else if (abs(id) == 14) {
          neutrinoLV.SetPxPyPzE(mcparticles.at(i).px,
                            mcparticles.at(i).py,
                            mcparticles.at(i).pz,
                            mcparticles.at(i).energy);
        }
