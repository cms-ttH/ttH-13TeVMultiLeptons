...
samples=( ["tth_powheg_old"]=19 )
# ["ttbar_semiLep_madgraph"]=122 )
...
for samp in "${!samples[@]}"
do
    sed -i 's/^sample = .*/sample = '"${samp}"'/' /afs/crc.nd.edu/user/c/cmuelle2/CMSSW_8_0_20/src/ttH-13TeVMultiLeptons/TemplateMakers/test/condor_test/run_2lss.submit
    sed -i 's/^N = .*/N = '"${samples[${samp}]}"'/' /afs/crc.nd.edu/user/c/cmuelle2/CMSSW_8_0_20/src/ttH-13TeVMultiLeptons/TemplateMakers/test/condor_test/run_2lss.submit
