void MakeGoodPlot::load_samples(std::vector<int> samps)
{
        ...
        TString basedir = "/mnt/ndpc6disk1/gesmith/crab/test/";     // The directory on ndpc disk where your samples are located.        
	...
	for (int i=0; i<numsamples; i++)
	{
		...
		if (samps[i]==5) // TTJets
		{
			basedir_plus = "try1_ttWkevin_mAODV2/TTJets/";                                              // <-- to update
		...
		if (samps[i]==6) // ZJets
		{
                        basedir_plus = "DYJetsToLL_M-50_13TeV-madgraph-pythia8/crab_zJets/150519_150537/0000/";     // <-- to update
			thesample = basedir + basedir_plus + "*.root";
			ch[6]->Add(thesample);    
                        xsecs[6]=2008.4;
                        numgen[6]=2829164.0;
		...
		if (samps[i]==7) // WJets
		{
                        basedir_plus = "WJetsToLNu_13TeV-madgraph-pythia8-tauola/crab_wJets/150519_150558/0000/";   // <-- to update
			thesample = basedir + basedir_plus + "*.root";
			ch[7]->Add(thesample);
                        xsecs[7]=20508.9;		
			numgen[7]=10017462.0;
		...
		{                        
                        basedir_plus = "ttWkevin/";                                                                 // <-- to update
                        thesample = basedir + basedir_plus + "*.root";
			ch[8]->Add(thesample);
			xsecs[8]=0.6647;
                        numgen[8]=246521.0;
		...
		{
                        basedir_plus = "TTZJets_Tune4C_13TeV-madgraph-tauola/crab_ttZjets/150519_150516/0000/";     // <-- to update
                        thesample = basedir + basedir_plus + "*.root";
			ch[9]->Add(thesample);
                        xsecs[9]=0.8565;
                        numgen[9]=249275.0;
		...
		{
                        basedir_plus = "WZJetsTo3LNu_Tune4C_13TeV-madgraph-tauola/crab_wzJets/150519_150616/0000/"; // <-- to update
                        thesample = basedir + basedir_plus + "*.root";
			ch[10]->Add(thesample);
                        xsecs[10]=2.165;
			numgen[10]=237484.0;
		...
		{
                        basedir_plus = "ZZTo4L_Tune4C_13TeV-powheg-pythia8/crab_zzJets/150519_150636/0000/";        // <-- to update
		...
		{
			basedir_plus = "TTJets_split_2lep/";                                                        // <-- to update
			thesample = basedir + basedir_plus + "*.root";
			ch[13]->Add(thesample);
		...
		{
			basedir_plus = "TTJets_split_1lepFromT/";                                                   // <-- to update
			thesample = basedir + basedir_plus + "*.root";
			ch[14]->Add(thesample);
		...
		{
			basedir_plus = "TTJets_split_1lepFromTBar/";                                                // <-- to update
			thesample = basedir + basedir_plus + "*.root";
			ch[15]->Add(thesample);
