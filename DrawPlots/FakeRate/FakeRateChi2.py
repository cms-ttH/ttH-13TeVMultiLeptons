...
def main():
    ...
    for bin in FR_keys:
        if FR_array[bin][0] == 1:
            ...
            FR_array[bin].append(config['initial FR']['tight mu']) #Initial FR err.
            FR_array[bin].append(config['initial FR']['tight mu']) #Final FR err.
        ...
        if FR_array[bin][0] == 0:
            ...
            FR_array[bin].append(config['initial FR']['tight ele']) #Initial FR err
            FR_array[bin].append(config['initial FR']['tight ele']) #Final FR err
    ...
    for bin in FR_keys:
        ...
        for lep_cat in config.get('lepton categories', {}).keys():
            ...
            for iLep in range(2):
                ...
                work.append( ('%s-%s-%d-%s-%s' % (bin, lep_cat, iLep, 'num',   'data'), pool.apply_async(compute_num_denom, ['data', bin, lep_cat, iLep, lepStr, base_cuts[lep_cat], WP_sel, WP_sel,    FR_array, config, bin] ) ) )
                work.append( ('%s-%s-%d-%s-%s' % (bin, lep_cat, iLep, 'num',   'MC'  ), pool.apply_async(compute_num_denom, ['MC',   bin, lep_cat, iLep, lepStr, base_cuts[lep_cat], WP_sel, WP_sel,    FR_array, config, bin] ) ) )
                work.append( ('%s-%s-%d-%s-%s' % (bin, lep_cat, iLep, 'num',   'QF'  ), pool.apply_async(compute_num_denom, ['QF',   bin, lep_cat, iLep, lepStr, base_cuts[lep_cat], WP_sel, WP_sel,    FR_array, config, bin] ) ) )
                work.append( ('%s-%s-%d-%s-%s' % (bin, lep_cat, iLep, 'denom', 'data'), pool.apply_async(compute_num_denom, ['data', bin, lep_cat, iLep, lepStr, base_cuts[lep_cat], WP_sel, WP_presel, FR_array, config, bin] ) ) )
                work.append( ('%s-%s-%d-%s-%s' % (bin, lep_cat, iLep, 'denom', 'MC'  ), pool.apply_async(compute_num_denom, ['MC',   bin, lep_cat, iLep, lepStr, base_cuts[lep_cat], WP_sel, WP_presel, FR_array, config, bin] ) ) )
                work.append( ('%s-%s-%d-%s-%s' % (bin, lep_cat, iLep, 'denom', 'QF'  ), pool.apply_async(compute_num_denom, ['QF',   bin, lep_cat, iLep, lepStr, base_cuts[lep_cat], WP_sel, WP_presel, FR_array, config, bin] ) ) )
...
#                 QF_yield[bin][lep_cat][iLep]['denom']   = compute_num_denom('QF',   bin, lep_cat, iLep, lepStr, base_cuts[lep_cat], WP_sel, WP_presel, FR_array, config, bin)
                ...
                for bin_NP in FR_keys:
                    ...
                    work.append( ('%s-%s-%d-%s-%s-%s' % (bin, lep_cat, iLep, 'num',   'NP_tag', bin_NP), pool.apply_async(compute_num_denom, ['NP_tag', bin, lep_cat, iLep, lepStr, base_cuts[lep_cat], WP_sel, WP_sel,    FR_array, config, bin_NP] ) ) )
                    work.append( ('%s-%s-%d-%s-%s-%s' % (bin, lep_cat, iLep, 'denom', 'NP_tag', bin_NP), pool.apply_async(compute_num_denom, ['NP_tag', bin, lep_cat, iLep, lepStr, base_cuts[lep_cat], WP_sel, WP_presel, FR_array, config, bin_NP] ) ) )
...
#                     NP_tag_yield[bin][lep_cat][iLep]['denom'][bin_NP] = compute_num_denom('NP_tag', bin, lep_cat, iLep, lepStr, base_cuts[lep_cat], WP_sel, WP_presel, FR_array, config, bin_NP)
    ...
    for (id, res) in work:
        ...
        str = id.split('-', 10)
        #print str
        #print '%s_yield[%s][%s][%s][%s]' % (str[4], str[0], str[1], str[2], str[3])
        if str[4] == 'data':
            data_yield[str[0]][str[1]][int(str[2])][str[3]] = res.get()
        if str[4] == 'MC':
            MC_yield[str[0]][str[1]][int(str[2])][str[3]] = res.get()
        if str[4] == 'QF':
            QF_yield[str[0]][str[1]][int(str[2])][str[3]] = res.get()
        if str[4] == 'NP_tag':
            NP_tag_yield[str[0]][str[1]][int(str[2])][str[3]][str[5]] = res.get()
...
#                 for bin_NP in FR_keys:
#                     NP_numerator += NP_tag_yield[bin][lep_cat][iLep]['num'][bin_NP]*FR_array[bin_NP][9]/(1-FR_array[bin_NP][9])
#                     NP_denominator += NP_tag_yield[bin][lep_cat][iLep]['denom'][bin_NP]*FR_array[bin_NP][9]/(1-FR_array[bin_NP][9])
...
#         print 'FR for bin '+bin+' = '+str(numerator/denominator)
    ...
    for bin_index in range(len(FR_keys)):
        print 'bin %s, index %d' % (FR_keys[bin_index], bin_index)
        FR_combos.append(numpy.arange(FR_array[FR_keys[bin_index]][7], FR_array[FR_keys[bin_index]][8]+(step_size/10), step_size))
        print FR_combos[bin_index]


    #FR_combos = numpy.arange(0.11, 0.37+(0.01/10), 0.01)
    FR_final = {}
    final_chi2 = 10000
    final_num = 0.0
    final_denom = 0.0
    ...
    #for FR_set in itertools.product(FR_combos):
    #for FR_set in itertools.product(FR_combos[0], FR_combos[1]):
    #for FR_set in itertools.product(FR_combos[0], FR_combos[1], FR_combos[2], FR_combos[3]):
    #for FR_set in itertools.product(FR_combos[0], FR_combos[1], FR_combos[2], FR_combos[3], FR_combos[4]):
    #for FR_set in itertools.product(FR_combos[0], FR_combos[1], FR_combos[2], FR_combos[3], FR_combos[4], FR_combos[5]):
    for FR_set in itertools.product(FR_combos[0], FR_combos[1], FR_combos[2], FR_combos[3], FR_combos[4], FR_combos[5], FR_combos[6]):
    #for FR_set in itertools.product(FR_combos[0], FR_combos[1], FR_combos[2], FR_combos[3], FR_combos[4], FR_combos[5], FR_combos[6], FR_combos[7]):
    #for FR_set in itertools.product(FR_combos[0], FR_combos[1], FR_combos[2], FR_combos[3], FR_combos[4], FR_combos[5], FR_combos[6], FR_combos[7], FR_combos[8], FR_combos[9]):
    #for FR_set in itertools.product(FR_combos, repeat=len(FR_keys)):
        if iii == 0:
            print FR_set
        if iii == 1000:
            jjj += 1
            print '%d iterations' % (iii*jjj)
            iii = 0
        iii += 1
        #print '--------- NEW FR SET -----------'
        #print FR_set
        ...
        in_range = True
        for bin_index in range(len(FR_keys)):
            FR_array[FR_keys[bin_index]][9] = FR_set[bin_index]
            if FR_set[bin_index] < FR_array[FR_keys[bin_index]][7]-(step_size/10) or FR_set[bin_index] > FR_array[FR_keys[bin_index]][8]+(step_size/10):
                print 'Not in range: %s with value %f where min = %f, max = %f' % (FR_keys[bin_index], FR_set[bin_index], FR_array[FR_keys[bin_index]][7], FR_array[FR_keys[bin_index]][8])
                print FR_set
                in_range = False
        if not in_range:
            continue
        ...
        for bin in FR_keys:
            ...
            for lep_cat in config.get('lepton categories', {}).keys():
                for iLep in range(2):
                    ...
                    for bin_NP in FR_keys:
                        numerator -= NP_tag_yield[bin][lep_cat][iLep]['num'][bin_NP]*FR_array[bin_NP][9]/(1-FR_array[bin_NP][9])
                        num_err2 += NP_tag_yield[bin][lep_cat][iLep]['num'][bin_NP]*FR_array[bin_NP][9]/(1-FR_array[bin_NP][9])
                        denominator -= NP_tag_yield[bin][lep_cat][iLep]['denom'][bin_NP]*FR_array[bin_NP][9]/(1-FR_array[bin_NP][9])
                        denom_err2 += NP_tag_yield[bin][lep_cat][iLep]['denom'][bin_NP]*FR_array[bin_NP][9]/(1-FR_array[bin_NP][9])
                    #print 'bin %s, lep_cat %s, iLep %d, num = %d, denom = %d, FR = %0.3f' % (bin, lep_cat, iLep, numerator, denominator, FR_array[bin][9])
                #print 'TOTAL bin %s, num = %d, denom = %d, FR = %0.3f' % (bin, numerator, denominator, FR_array[bin][9])
            #numerator = max(numerator, 1.0)
            #denominator = max(denominator, 1.0)
            this_chi2 += pow( ( (numerator/denominator) - FR_array[bin][9] ) / (math.sqrt(num_err2/pow(numerator,2) + denom_err2/pow(denominator,2))*numerator/denominator) , 2)
            FR_array[bin][11] = math.sqrt(num_err2/pow(numerator,2) + denom_err2/pow(denominator,2))*numerator/denominator
            this_num += numerator
            this_denom += denominator
            if denominator < 0:
                this_chi2 += 1000
            #print 'bin %s this_chi2 = %0.3f' % (bin, this_chi2)

        if this_chi2 < final_chi2:
            final_chi2 = this_chi2
            final_num = this_num
            final_denom = this_denom
            FR_final = FR_set
            for bin in FR_keys:
                FR_array[bin][10] = FR_array[bin][9]
                FR_array[bin][12] = FR_array[bin][11]

    print 'Minimum chi2 = %0.3f, Ndof = %d' % (final_chi2, len(FR_keys))
    print 'Numerator = %0.1f, denominator = %0.1f' % (final_num, final_denom)
    print 'chi2 / Ndof = %0.3f' % (final_chi2/len(FR_keys))
    print FR_final
    for bin in FR_keys:
        print '%s, %0.3f +/- %0.3f' % (bin, FR_array[bin][10], FR_array[bin][12])

                    


        #FR_array[bin][10] = num/denom
    #print 'DATA YIELD'
    #print data_yield
    #print 'MC YIELD'
    #print MC_yield
...
#             FR_hist_tight_mu_final.SetBinContent(histBin, FR_array[bin][10])
#         if FR_array[bin][0] == 0:
...
#             FR_hist_tight_ele_final.SetBinContent(histBin, FR_array[bin][10])
