...
ConfigParser -- responsible for parsing a list of
	...
	__init__(defaults=None, dict_type=_default_dict, allow_no_value=True,
		...
		When `allow_no_value' is True (default: True), options without
...
class RawConfigParser(MutableMapping):
	...
	def __init__(self, defaults=None, dict_type=_default_dict,
				 allow_no_value=True, **_3to2kwargs):
...
#AW		   return self._proxies[key].keys()
...
class SectionProxy(MutableMapping):
	...
	def __getitem__(self, key):
		if not self._parser.has_option(self._name, key):
			raise KeyError(key)
		return self.get(key)
#AW		   return self._parser.get(self._name, key)	   
	...
	def get(self, option, fallback=None, **_3to2kwargs):
		...
		## AWB - if entry is [" , "] it is not a list. [" " ,] or [, " "] is a list.
		...
		if ',' in entry and ( not entry.startswith('"') or entry_first_doublequote_removed.find('"') < entry_first_doublequote_removed.find(',')):			
			...
			for value in string_list:
				...
				if not re.search('[a-zA-Z]', value) and not value.startswith('"') and not value.endswith('"'):
					if '.' in value:
						...
						value = int(value)
				elif 'true' in value or 'True' in value:
					value = bool(True)
